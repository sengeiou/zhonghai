...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = ASLocalizedString(@"KDVoteViewController_navigationItem_title");
    //    [self setUpVoteView];
...
- (void)showHasBeenDelectView {
    ...
    [[[KDAccountTipView alloc] initWithTitle:@"" message:ASLocalizedString(@"KDVoteViewController_Vote_Delete")buttonTitle:ASLocalizedString(@"Global_Sure")completeBlock:^{
        [self.navigationController popViewControllerAnimated:YES];
    }]  showWithType:KDAccountTipViewTypeAlert window:self.view.window];
}
...
- (void)alterShareVoteSuccess {
    UIAlertView *alterViewe = [[UIAlertView alloc] initWithTitle:ASLocalizedString(@"KDVoteViewController_Vote_Success")message:ASLocalizedString(@"KDVoteViewController_Tips")delegate:self cancelButtonTitle:ASLocalizedString(@"KDVoteViewController_NO")otherButtonTitles:ASLocalizedString(@"KDVoteViewController_Share"), nil];
    [alterViewe show];
//    [alterViewe release];
}
...
- (void)alterShareVoteHasEnd {
    UIAlertView *alterViewe = [[UIAlertView alloc] initWithTitle:@""
                                                         message:ASLocalizedString(@"KDVoteViewController_Ending")delegate:nil cancelButtonTitle:ASLocalizedString(@"Global_Sure")otherButtonTitles:nil, nil];
    [alterViewe show];
//    [alterViewe release];
}
        ...
        if([response isValidResponse]) {
            ...
            if (![response isCancelled]) {
                if (404 == [response statusCode]) {
                    ...
                    UIAlertView *alterViewe = [[UIAlertView alloc] initWithTitle:@""
                                                                         message:ASLocalizedString(@"KDVoteViewController_Err")delegate:vvc cancelButtonTitle:ASLocalizedString(@"Global_Sure")otherButtonTitles:nil, nil];
                    [alterViewe show];
//                    [alterViewe release];
                }
            }
...
- (void)voteShare {
    ...
    KDServiceActionDidCompleteBlock completionBlock = ^(id results, KDRequestWrapper *request, KDResponseWrapper *response){
        if ([(NSNumber *)results boolValue]) {
            [[[KDWeiboAppDelegate getAppDelegate] getOverlay] postFinishMessage:ASLocalizedString(@"KDSheet_Share_Success")duration:2.0 animated:YES];
        }
