//
//  SignInShareRangeViewController.m
//  kdweibo
//
//  Created by Tan yingqi on 13-10-16.
//  Copyright (c) 2013年 www.kingdee.com. All rights reserved.
//

#import "KDSearchBar.h"
#import "SignInShareRangeViewController.h"
...
@interface SignInShareRangeViewController ()<KDSearchBarDelegate,UIAlertViewDelegate> {
...
@implementation SignInShareRangeViewController
...
- (void)dealloc {
    ...
    //KD_RELEASE_SAFELY(fileteredGroups_);
    //KD_RELEASE_SAFELY(groups_);
    //KD_RELEASE_SAFELY(unGroups_);
    //KD_RELEASE_SAFELY(allGroups_);
...
-(UITableViewCell *)companyCell {
    if (!companyCell_) {
        ...
        //[companyCell_ addBorderAtPosition:KDBorderPositionBottom];
...
-(UITableViewCell *)groupCell {
    if (!groupCell_) {
        ...
        UILabel *labelLeft = [[UILabel alloc] initWithFrame:CGRectZero];// autorelease];
        labelLeft.text = ASLocalizedString(@"SignInShareRangeViewController_labelLeft_text");
        labelLeft.font = [UIFont systemFontOfSize:13.0f];
        labelLeft.textColor = MESSAGE_DATE_COLOR;
        labelLeft.backgroundColor = [UIColor clearColor];
        [labelLeft sizeToFit];
        
        groupCell_.accessoryView = labelLeft;
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        self.navigationItem.title = ASLocalizedString(@"SignInShareRangeViewController_navigationItem_title");
...
//    [tableView release];
    ...
    //    [sendButton setBackgroundImage:[UIImage imageNamed:@"post_send_btn_bg_v2.png"] forState:UIControlStateNormal];
    //    [sendButton setBackgroundImage:[UIImage imageNamed:@"post_send_btn_bg_hl_v2.png"] forState:UIControlStateHighlighted];
...
- (void)confirm:(id)sender {
    [self updateShareRange];
}

- (void)reloadDataSource{
    
    [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    __block SignInShareRangeViewController *weakSelf = self;// retain];
    KDServiceActionDidCompleteBlock completionBlock = ^(id results, KDRequestWrapper *request, KDResponseWrapper *response){
        ...
        if(results) {
            ...
            if (success) {
                ...
                weakSelf->isCompanySeleted = [(NSNumber *)[returnDic objectForKey:@"isCompanySelected"] boolValue];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        isCompanySeleted = !isCompanySeleted;
        ...
        if (indexPath.section == 2) {
            ...
            if ([self.groups containsObject:group]) {
                ...
                [self.unGroups addObject:group];
            }else {
                if ([self.groups count] >= 3) {
                    [[KDNotificationView defaultMessageNotificationView] showInView: self.view message:ASLocalizedString(@"SignInShareRangeViewController_Alert")type:KDNotificationViewTypeNormal];
                    return;
                }
                [self.unGroups removeObject:group];
...
- (void)updateShareRange {
    ...
    if ([self.groups count] >0) {
        ...
        NSUInteger idx = 0;
        for (KDGroup *group in self.groups) {
            [IDs appendString:group.groupId];
        ...
        groups = IDs;
        if (isCompanySeleted) {
            groups = [groups stringByAppendingFormat:@",-1"];
        }
    }else {
        if (isCompanySeleted) {
            groups = @"-1";
    ...
    [query setParameter:@"groups" stringValue:groups];
    __block SignInShareRangeViewController *weakSelf = self;// retain];
    KDServiceActionDidCompleteBlock completionBlock = ^(id results, KDRequestWrapper *request, KDResponseWrapper *response){
        NSString *errorMessage = nil;
        if ([response isValidResponse]) {
            NSDictionary *returnDic = results;
            NSNumber *num = [returnDic objectNotNSNullForKey:@"success"];
            errorMessage = [returnDic objectNotNSNullForKey:@"errorMessage"];
            if (num) {
                ...
                errorMessage = ASLocalizedString(@"无法解析");
            }
        }else {
            errorMessage = [[response responseDiagnosis] networkErrorMessage];
        }
        ...
        if ([weakSelf.view superview]) {
            [[MBProgressHUD HUDForView:weakSelf.view] hide:YES];
        }
//        [weakSelf release];
    };
    
    [KDServiceActionInvoker invokeWithSender:weakSelf actionPath:@"/signId/:updateSignInRangeShare" query:query
                                 configBlock:nil completionBlock:completionBlock];
