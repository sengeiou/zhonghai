...
@interface KDAlertWindowOverlay : UIView
@property (nonatomic, retain) KDCheckBoxAlertView2 *alertView;
@end

@interface KDAlertViewButton : UIButton
@end

@interface KDCheckBoxAlertView2 ()
@property (nonatomic, retain) NSMutableArray *buttons;
@property (nonatomic, retain) UIFont *titleFont;
@property (nonatomic, retain) UIFont *subtitleFont;
@property (nonatomic, retain) KDAlertWindowOverlay *overlay;
@property (nonatomic, assign) kKDAlertAnimation animationType;
@property (nonatomic, copy) kKDAlertViewBlock block;
@property (nonatomic, retain) UIView *blurredBackgroundView;
@property (nonatomic, retain) UIView *checkView;
@property (nonatomic, retain) UIButton *checkButton;
- (CGRect)defaultFrame;
- (void)animateWithType:(kKDAlertAnimation)animation show:(BOOL)show completionBlock:(void(^)())completion;
...
#define kkKDAlertBackgroundRadius 10.0
#define kkKDAlertFrameInset 7.0
#define kkKDAlertPadding 10.0
#define kkKDAlertButtonPadding 6.0
#define kkKDAlertButtonHeight 44.0
#define kkKDAlertButtonOffset 66.5

@implementation KDCheckBoxAlertView2 {
@private
	struct {
		CGRect titleRect;
		CGRect subtitleRect;
...
+ (KDCheckBoxAlertView2 *)dialogWithTitle:(NSString *)title subtitle:(NSString *)subtitle {
    return [[KDCheckBoxAlertView2 alloc] initWithTitle:title subtitle:subtitle];// autorelease];
}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.animationType = kKDAlertAnimationDefault;
		self.buttons = [NSMutableArray array];
		
		self.opaque = NO;
		self.alpha = 1.0;
		self.darkenBackground = YES;
		self.blkKDackground = YES;
        ...
        [_checkButton setTitle:ASLocalizedString(@"KDCheckBoxAlertView2_never_prompt")forState:UIControlStateNormal];
...
- (NSInteger)addButtonWithTitle:(NSString *)title {
	...
	KDAlertViewButton *button = [[KDAlertViewButton alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, kkKDAlertButtonHeight)];
...
- (void)layoutSubviews {
	...
	CGFloat layoutFrameInset = kkKDAlertFrameInset + kkKDAlertPadding;
	...
	if (self.buttons.count > 0) {
		buttonsHeight = kkKDAlertButtonHeight;
	}
	CGFloat buttonRegionPadding = ((kkKDAlertButtonOffset - kkKDAlertFrameInset) - kkKDAlertButtonHeight) / 2.0 - 2.0;
	layout.buttonRect = CGRectMake(CGRectGetMinX(layoutFrame), CGRectGetMaxY(self.bounds) - kkKDAlertButtonOffset + buttonRegionPadding + _checkView.frame.size.height, layoutWidth, buttonsHeight);
	...
	if (count > 0) {
		CGFloat buttonWidth = (CGRectGetWidth(layout.buttonRect) - kkKDAlertButtonPadding * ((CGFloat)count - 1.0)) / (CGFloat)count;
		for (int i = 0; i < count; i++) {
			CGFloat xOffset = CGRectGetMinX(layout.buttonRect) + (kkKDAlertButtonPadding + buttonWidth) * (CGFloat)i;
			...
			KDAlertViewButton *button = (KDAlertViewButton *)[self.buttons objectAtIndex:i];
			button.frame = frame;
    ...
    dialogFrame.size.height += _checkView.frame.size.height;
	dialogFrame.origin.x = (CGRectGetWidth(screenFrame) - CGRectGetWidth(dialogFrame)) / 2.0;
	dialogFrame.origin.y = (CGRectGetHeight(screenFrame) - CGRectGetHeight(dialogFrame)) / 2.0;
    dialogFrame.origin.y -= _checkView.frame.size.height;
	self.frame = CGRectIntegral(dialogFrame);
    
    CGRect checkFrame = _checkView.frame;
    
    checkFrame.origin.y = CGRectGetMinY(layout.buttonRect) - 40.f;
...
- (void)drawRect:(CGRect)rect {
	...
	CGFloat cornerRadius = kkKDAlertBackgroundRadius;
	CGRect pathFrame = CGRectInset(self.bounds, kkKDAlertFrameInset, kkKDAlertFrameInset);
	...
	CGFloat buttonOffset = activeBounds.size.height - kkKDAlertButtonOffset; // offset buttonOffset by half point for crisp lines
...
- (void)buttonTapped:(id)button {
	NSUInteger buttonIndex = [self.buttons indexOfObject:(KDAlertViewButton *)button];
	if (self.block)
...
- (UIView *)blurredBackground {
    ...
    UIImageView *blurredView = [[UIImageView alloc] initWithFrame:backgroundView.bounds];// autorelease];
...
- (void)animateWithType:(kKDAlertAnimation)animation show:(BOOL)show completionBlock:(void (^)())completion {
	...
	// fade animation
	if (animation == kKDAlertAnimationFade) {
		if (show) {
	...
	else if (animation == kKDAlertAnimationFlipHorizontal || animation == kKDAlertAnimationFlipVertical) {
		
		CGFloat xAxis = (animation == kKDAlertAnimationFlipVertical) ? 1.0 : 0.0;
		CGFloat yAxis = (animation == kKDAlertAnimationFlipHorizontal) ? 1.0 : 0.0;
	...
	// tumble animation
	else if (animation == kKDAlertAnimationTumble) {
		if (show) {
			[self showOverlay:YES];
	...
	// slide animation
	else if (animation == kKDAlertAnimationSlideLeft || animation == kKDAlertAnimationSlideRight) {
		if (show) {
			[self showOverlay:YES];
			
			CGFloat startX = (animation == kKDAlertAnimationSlideLeft) ? 200.0 : -200.0;
			CGFloat shiftX = 5.0;
			if (animation == kKDAlertAnimationSlideLeft)
				shiftX *= -1.0;
			...
			CGFloat finalX = (animation == kKDAlertAnimationSlideLeft) ? -400.0 : 400.0;
			CGFloat shiftX = 5.0;
			if (animation == kKDAlertAnimationSlideRight)
				shiftX *= 1.0;
...
//			self.overlay = [kKDAlertWindowOverlay new];
//			kKDAlertWindowOverlay *overlay = self.overlay;
//			overlay.opaque = NO;
...
- (void)dealloc
{
    ...
    //KD_RELEASE_SAFELY(_subtitle);
    //KD_RELEASE_SAFELY(_buttons);
    //KD_RELEASE_SAFELY(_titleFont);
    //KD_RELEASE_SAFELY(_overlay);
    //KD_RELEASE_SAFELY(_subtitle);
    //KD_RELEASE_SAFELY(_blurredBackgroundView);
...
#pragma mark - kKDAlertWindowOverlay

@implementation KDAlertWindowOverlay
...
- (void)drawRect:(CGRect)rect {
	...
	CGGradientRef radialGradient = CGGradientCreateWithColors(colorSpace, (CFArrayRef)radialGradientColors, radialGradientLocations);
...
#pragma mark - kKDAlertViewButton

@implementation KDAlertViewButton
...
- (UIImage *)normalButtonImage {
	...
	CGGradientRef buttonBgGradient = CGGradientCreateWithColors(colorSpace, (CFArrayRef)buttonBgGradientColors, buttonBgGradientLocations);
	...
	CGGradientRef buttonOuterGradient = CGGradientCreateWithColors(colorSpace, (CFArrayRef)buttonOuterGradientColors, buttonOuterGradientLocations);
	...
	CGGradientRef buttonInnerGradient = CGGradientCreateWithColors(colorSpace, (CFArrayRef)buttonInnerGradientColors, buttonInnerGradientLocations);
