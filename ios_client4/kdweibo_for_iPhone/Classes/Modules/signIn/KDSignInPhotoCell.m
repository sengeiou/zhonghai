...
@implementation KDSignInPhotoCell
...
- (NSString *)fileSizeStrForImages
{
    ...
    if(num<=1024)
    {
        return  [NSString stringWithFormat:ASLocalizedString(@"图片总大小:%lld字节"),num];
    }else if(num>1024 && num < 1024*1024)
    {
        CGFloat size = num/1024.0;
        return [NSString stringWithFormat:ASLocalizedString(@"图片总大小:%.2fKB"),size];
    }else {
        CGFloat size = num/(1024.0*1024.0);
        return [NSString stringWithFormat:ASLocalizedString(@"图片总大小:%.2fM"),size];
