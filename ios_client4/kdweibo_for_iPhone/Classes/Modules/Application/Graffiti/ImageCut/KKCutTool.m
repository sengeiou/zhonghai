...
- (void)setMenu {
    UIButton *btn = [self buttonWithTitle:@"旋转" image:[UIImage imageNamed:@"graffiti_rotate"]];
    btn.frame = CGRectMake(20, 20, 75, 40);
    btn.center = CGPointMake([UIScreen mainScreen].bounds.size.width/3, 18);
    [btn addTarget:self action:@selector(rotate) forControlEvents:UIControlEventTouchUpInside];
    [_menuContainer addSubview:btn];
    
    UIButton *btnRestore = [self buttonWithTitle:@"还原" image:[UIImage imageNamed:@"graffiti_rotateReturn"]];
    btnRestore.frame = CGRectMake(120, 20, 75, 40);
    btnRestore.center = CGPointMake([UIScreen mainScreen].bounds.size.width/3 * 2, 18);
    ...
    UIButton *okBtn= [[UIButton alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width - 12 - 75, 36+8, 75, 26)];
    okBtn.backgroundColor = [UIColor redColor];
    [okBtn setTitle:@"裁剪" forState:UIControlStateNormal];
