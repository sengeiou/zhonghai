...
//4个角的拖动圆球
- (KKCutConer*)clippingCircleWithTag:(NSInteger)tag
{
    KKCutConer *view = [[KKCutConer alloc] initWithFrame:CGRectMake(0, 0, 75, 75) location:tag];
    view.tag = tag;
...
- (void)panGridView:(UIPanGestureRecognizer*)sender
{
    ...
    if(sender.state==UIGestureRecognizerStateBegan){
        ...
        CGFloat height = MIN(self.frame.size.height - top, initialRect.size.height);
        CGFloat width =  MIN(self.frame.size.width - left, initialRect.size.width);
