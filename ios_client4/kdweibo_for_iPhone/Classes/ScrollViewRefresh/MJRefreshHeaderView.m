...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.releaseToRefreshText = ASLocalizedString(@"MJRefreshConst_HeaderReleaseToRefresh");
        self.refreshingText = ASLocalizedString(@"MJRefreshConst_HeaderRefreshing");
    }
    return self;
}
...
- (void)updateTimeLabel
{
    ...
    if ([cmp1 day] == [cmp2 day]) { // 今天
        formatter.dateFormat = ASLocalizedString(@"MJRefreshHeaderView_date");
    ...
    // 3.显示日期
    self.lastUpdateTimeLabel.text = [NSString stringWithFormat:ASLocalizedString(@"MJRefreshHeaderView_lastUpdateTimeLabel_text"), time];
}
...
#warning 这句代码修复了，top值不断累加的bug
                    //无奈之举，只能写死
                    self.scrollView.mj_contentInsetTop = 0;//self.mj_height-64;
                }];
            } else {
                // 执行动画
