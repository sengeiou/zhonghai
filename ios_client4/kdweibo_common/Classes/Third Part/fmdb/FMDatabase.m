...
//        [self setKey:encryptKey_];
        if(![self goodConnection]){ //无效连接,即为旧数据，进行加密
            [self close];
            [FMEncryptHelper encryptDatabase:[NSString stringWithUTF8String:(_databasePath ? [_databasePath fileSystemRepresentation] : ":memory:")]];
            [self open];
        }
    }
