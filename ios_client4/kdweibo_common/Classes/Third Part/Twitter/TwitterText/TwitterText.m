...
#define TWUValidHashtag \
    @"(?:" KDUHashtagBoundary @")([#＃]" TWUHashtagAlphanumeric @"*" TWHashtagAlpha TWUHashtagAlphanumeric @"*[#＃])"

#define TWUEndHashTagMatch      @"\\A(?:[#＃])"

#define KDWeiboHashtagPattern      @"#([^#\\\\]+?)#" // from kdweibo server (not good enough)
...
#define TWUValidMentionPrecedingChars   @"(?:[^a-zA-Z0-9_!#$%&*@＠]|^)"
#define TWUAtSigns                      @"[@＠]"
...
#define TWUValidMentionOrList \
    ...
    @"(" TWUAtSigns @")" \
    @"([\u4e00-\u9fa5]{2,20}|[_.a-zA-Z0-9]{4,20}|[\u4e00-\u9fa5_.a-zA-Z0-9]{4,20})"

#define TWUValidReply                   @"\\A(?:[" TWUUnicodeSpaces @"])*" TWUAtSigns @"([a-zA-Z0-9_]{1,20})"
#define TWUEndMentionMatch              @"\\A(?:" TWUAtSigns @"|[" TWULatinAccents @"])"


//#define KDWeiboScreenNamePattern      TWUAtSigns @"([a-zA-Z0-9]{0,20}[\u4e00-\u9fa5]{0,20}[_.a-zA-Z0-9]{0,20}){2,20}"   // (not good enough)
#define KDWeiboScreenNamePattern @"@[^\\s\\$\\^\\[\\]\\?:!#%&=;@'\"<>\f$，；：“”。！、？]+"

#define TWUHighlightKeywords            ASLocalizedString(@"[今明后]天|[上中下]午|[早晚]上|开会")//
// URL
//

/////@Modify : shenkuikui 2013/04/08 18:01. (用于修正话题后紧跟url)
...
@"(?:" \
    ...
    @"(?:" TWUValidGTLD @"|" TWUValidCCTLD @"|" TWUValidPunycode @"|" @"[0-9]*" @")" \
@")"
...
//for kingdee
#define KDValidURL @"http://([\\w-]+\\.)+[\\w-]+(/[a-z,A-Z,0-9,_./?%]*)?"
#define KDValidPhoneNumber @"(^1\\d{10}$)|(^0\\d{10,11}$)|(^1\\d{2}-\\d{4}-\\d{4}$)|(^\\d{3,4}-\\d{7,8}$)|(^\\d{7,8}$)"
#define KDValidEmail @"\\w+([-+.]\\w+)*@\\w+([-.]\\w+)*\\.\\w+([-.]\\w+)*"

static const int MaxTweetLength = 1000;
static const int HTTPShortURLLength = 10;
static const int HTTPSShortURLLength = 10;
...
static NSRegularExpression *validKeywordRegexp;
...
static NSRegularExpression *validPhoneNumberRegexp;
static NSRegularExpression *validEmailRegexp;
...
+ (NSArray*)phoneNumberInText:(NSString*)text withURLEntities:(NSArray*)urlEntities
{
    if (!text.length) {
        return [NSArray array];
    }
    
    if (!validPhoneNumberRegexp) {
        validPhoneNumberRegexp = [[NSRegularExpression alloc] initWithPattern:KDValidPhoneNumber options:NSRegularExpressionCaseInsensitive error:NULL];
    }
    ...
    while (1) {
        NSTextCheckingResult *matchResult = [validPhoneNumberRegexp firstMatchInString:text options:NSMatchingWithoutAnchoringBounds range:NSMakeRange(position, len - position)];
        if (!matchResult || matchResult.numberOfRanges < 2) {
            break;
        }
        
        NSRange phoneNumberRange = matchResult.range;
        BOOL matchOk = YES;
        
        // Check URL overlap
        for (TwitterTextEntity *urlEntity in urlEntities) {
            if (NSIntersectionRange(urlEntity.range, phoneNumberRange).length > 0) {
                matchOk = NO;
                break;
            }
        }
        ...
        if (matchOk) {
            TwitterTextEntity *entity = [TwitterTextEntity entityWithType:TwitterTextEntityPhoneNumber range:phoneNumberRange];
...
+ (NSArray*)emailInText:(NSString*)text withURLEntities:(NSArray*)urlEntities
{
    if (!text.length) {
        return [NSArray array];
    }
    ...
    if (!validEmailRegexp) {
        validEmailRegexp = [[NSRegularExpression alloc] initWithPattern:KDValidEmail options:NSRegularExpressionCaseInsensitive error:NULL];
    }
    ...
    while (1) {
        NSTextCheckingResult *matchResult = [validEmailRegexp firstMatchInString:text options:NSMatchingWithoutAnchoringBounds range:NSMakeRange(position, len - position)];
        if (!matchResult || matchResult.numberOfRanges < 2) {
            break;
        }
        ...
        if (matchOk) {
            TwitterTextEntity *entity = [TwitterTextEntity entityWithType:TwitterTextEntityEmail range:phoneNumberRange];
            [results addObject:entity];
        }
...
+ (NSArray*)entitiesInText:(NSString*)text
{
    ...
    NSArray *phoneNumbers = [self phoneNumberInText:text withURLEntities:urls];
    [results addObjectsFromArray:phoneNumbers];
    
    NSArray *emails = [self emailInText:text withURLEntities:urls];
    [results addObjectsFromArray:emails];
    
    NSArray *keywords = [self keywordsInText:text withURLEntities:urls];
    [results addObjectsFromArray:keywords];
...
+ (NSArray*)URLsInText:(NSString*)text
{
    ...
    if (!validURLRegexp) {
        //modified at 2013年12月17日10:29:58 by shenkuikui。.
        //reason:ip地址需要解析为连接。.
        //modified at 2014年01月14日16:44:02 by sehnkuikui. roll back!
...
//        validURLRegexp = [[NSRegularExpression alloc] initWithPattern:KDValidURL options:NSRegularExpressionCaseInsensitive error:NULL];
    }
...
+ (NSArray*)hashtagsInText:(NSString*)text withURLEntities:(NSArray*)urlEntities
{
    ...
    if (!validHashtagRegexp) {
        validHashtagRegexp = [[NSRegularExpression alloc] initWithPattern:KDWeiboHashtagPattern options:NSRegularExpressionCaseInsensitive error:NULL];
    }
    ...
    while (1) {
        ...
        NSRange hashtagRange = matchResult.range;
        BOOL matchOk = YES;
...
+ (NSArray*)keywordsInText:(NSString*)text withURLEntities:(NSArray*)urlEntities
{
    if (!text.length) {
        return [NSArray array];
    }

    if (!validKeywordRegexp) {
        validKeywordRegexp = [[NSRegularExpression alloc] initWithPattern:TWUHighlightKeywords options:NSRegularExpressionCaseInsensitive error:NULL];
    }
    
    NSMutableArray *results = [NSMutableArray array];
    NSInteger len = text.length;
    NSInteger position = 0;
    NSArray *matches = [validKeywordRegexp matchesInString:text options:0 range:NSMakeRange(position, len)];
    ...
    for (NSTextCheckingResult *result in matches)
    {
        ...
        // Check URL overlap
        for (TwitterTextEntity *urlEntity in urlEntities) {
            if (NSIntersectionRange(urlEntity.range, result.range).length > 0) {
                matchOk = NO;
                break;
            }
        }
        if (matchOk) {
            TwitterTextEntity *entity = [TwitterTextEntity entityWithType:TwitterTextEntityKeyword range:result.range];
            [results addObject:entity];
        }
    }
    return results;
...
+ (NSArray*)mentionsOrListsInText:(NSString*)text
{
    ...
    if (!validMentionOrListRegexp) {
        validMentionOrListRegexp = [[NSRegularExpression alloc] initWithPattern:KDWeiboScreenNamePattern options:NSRegularExpressionCaseInsensitive error:NULL];
    }
    ...
    while (1) {
        ...
        TwitterTextEntity *entity = [TwitterTextEntity entityWithType:TwitterTextEntityScreenName range:allRange];
        [results addObject:entity];
...
+ (int)tweetLength:(NSString*)text httpURLLength:(int)httpURLLength httpsURLLength:(int)httpsURLLength
{
    ...
    for (int i=(int)urlEntities.count-1; i>=0; i--) {
...
+ (int)remainingCharacterCount:(NSString*)text inMaxCount:(int) count {
    return [self remainingCharacterCount:text inMaxCount:count httpURLLength:HTTPShortURLLength httpsURLLength:HTTPSShortURLLength];
}
+ (int)remainingCharacterCount:(NSString*)text inMaxCount:(int) count  httpURLLength:(int)httpURLLength httpsURLLength:(int)httpsURLLength {
    return count - [self tweetLength:text httpURLLength:httpURLLength httpsURLLength:httpsURLLength];
