...
- (void)storeImage:(UIImage *)image forKey:(NSString *)key1 recalculateFromImage:(BOOL)recalculate imageData:(NSData *)imageData forKey:(NSString *)key2 toDisk:(BOOL)toDisk {
    if (!image || !key1) {
        return;
    }

    [self.memCache setObject:image forKey:key1 cost:image.size.height * image.size.width * image.scale];
...
//            if (image && (recalculate || !data)) {
            if (image) {
                ...
                if (data) {
                    ...
                    [_fileManager createFileAtPath:[self defaultCachePathForKey:key1] contents:data attributes:nil];
                }
            }
            ...
            if (imageData) {
                ...
                [_fileManager createFileAtPath:[self defaultCachePathForKey:key2] contents:imageData attributes:nil];
            }
        });
    }
}
...
- (void)storeImage:(UIImage *)image forKey:(NSString *)key {
    [self storeImage:image forKey:key recalculateFromImage:YES imageData:nil forKey:nil toDisk:YES];
}

- (void)storeImage:(UIImage *)image forKey:(NSString *)key toDisk:(BOOL)toDisk {
    [self storeImage:image forKey:key recalculateFromImage:YES imageData:nil forKey:nil toDisk:toDisk];
}
...
- (NSOperation *)queryDiskDataCacheForKey:(NSString *)key done:(void (^)(NSData *image, SDImageCacheType cacheType))doneBlock {
    ...
    dispatch_async(self.ioQueue, ^{
        ...
        @autoreleasepool {
            NSData *diskImage = [self diskImageDataForKey:key];
