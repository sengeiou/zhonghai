...
- (NSArray *)oauthGenerateTimestampAndNonce
{
    ...
    do {
        ...
        timestamp = [NSString stringWithFormat:@"%ld", (long)tv.tv_sec];
...
//    NSAssert1([self.url host] != nil, @"URL host missing: %@", [self.url absoluteString]);
    ...
    if (pathEnd.location != NSNotFound) {
        ...
        // There is an error at below code,
        // Think about if the range.location == NSNotFound and it means try to retrieve sub string from index NSNotFound (NSIntegerMax)
