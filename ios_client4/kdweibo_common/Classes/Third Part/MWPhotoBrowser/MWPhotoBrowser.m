...
- (void)viewDidLoad
{
    ...
    float fScreenWidth = [UIScreen mainScreen].bounds.size.width;
    
    buttonImageCheckbox = [[UIButton alloc]initWithFrame:CGRectMake(15, fScreenHeight-31, 18, 18)];
    [buttonImageCheckbox setImage:[UIImage imageNamed:@"Common.bundle/Images/image_picker_original_uncheck"] forState:UIControlStateNormal];
    [buttonImageCheckbox setImage:[UIImage imageNamed:@"Common.bundle/Images/image_picker_original_check"] forState:UIControlStateSelected];
//    [buttonImageCheckbox addTarget:self action:@selector(buttonCheckboxPressed:) forControlEvents:UIControlEventTouchUpInside];
    
    buttonImageCheckboxLarger = [[UIButton alloc]initWithFrame:CGRectMake(15, fScreenHeight - 44, 120, 44)];
    [buttonImageCheckboxLarger addTarget:self action:@selector(buttonCheckboxPressed:) forControlEvents:UIControlEventTouchUpInside];
    
    labelImageSize = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(buttonImageCheckbox.frame)+8, fScreenHeight-44, 100, 44)];
    labelImageSize.backgroundColor = [UIColor clearColor];
    labelImageSize.text = ASLocalizedString(@"原图(321KB)");
    labelImageSize.textColor = [UIColor whiteColor];
    labelImageSize.font = [UIFont systemFontOfSize:13];
    labelImageSize.alpha = buttonImageCheckbox.selected ? 1 : 1;
    buttonImageCheckbox.alpha = buttonImageCheckbox.selected ? 1 : 1;

    buttonSend = [[UIButton alloc]initWithFrame:CGRectMake(fScreenWidth-70, fScreenHeight-43, 70, 44)];
    [buttonSend setTitle:ASLocalizedString(@"Global_Send")forState:UIControlStateNormal];
    [buttonSend.titleLabel setFont:[UIFont boldSystemFontOfSize:16]];
    [buttonSend addTarget:self action:@selector(buttonSendPresssed:) forControlEvents:UIControlEventTouchUpInside];
    [buttonSend setTitleColor:[self KDBlueColor] forState:UIControlStateNormal];
    
//    SetBorder(buttonSend, [UIColor whiteColor]);
    buttonSendSum = [[UIButton alloc]initWithFrame:CGRectMake(fScreenWidth-75, fScreenHeight-30.5, 18, 18)];
    [buttonSendSum setTitle:@"0" forState:UIControlStateNormal];
    [buttonSendSum.titleLabel setFont:[UIFont systemFontOfSize:12]];
    buttonSendSum.titleEdgeInsets = UIEdgeInsetsMake(1, 1, 0, 0);
    [buttonSendSum setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [buttonSendSum setBackgroundImage:[UIImage imageNamed:@"Common.bundle/Images/image_picker_sum_bg"]  forState:UIControlStateNormal];
//    SetBorder(buttonSendSum, [UIColor whiteColor]);

    viewToolbarBG = [[UIView alloc]initWithFrame:CGRectMake(0, fScreenHeight-44, fScreenWidth, 44)];
    viewToolbarBG.backgroundColor = [UIColor blackColor];
    viewToolbarBG.alpha = .8;
    
    viewNavigationBarBG = [[UIView alloc]initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 64)];
    viewNavigationBarBG.backgroundColor = [UIColor blackColor];
    viewNavigationBarBG.alpha = .7;
    
    buttonBack = [[UIButton alloc]initWithFrame:CGRectMake(0, 20, 56, 44)];
    [buttonBack addTarget:self action:@selector(buttonBackPressed) forControlEvents:UIControlEventTouchUpInside];
    [buttonBack setImage:[UIImage imageNamed:@"Common.bundle/Images/photos_back_btn_bg.png"] forState:UIControlStateNormal];
    [buttonBack setImage:[UIImage imageNamed:@"Common.bundle/Images/photos_back_btn_bg_hl.png"] forState:UIControlStateHighlighted];
    buttonBack.imageView.contentMode = UIViewContentModeScaleAspectFit;
    
    
    labelTitle = [[UILabel alloc]initWithFrame:CGRectMake(0, 20, 100, 44)];
    SetCenterX(labelTitle.center, fScreenWidth/2);
    ...
    [self.view addSubview:viewToolbarBG];
    
    [self.view addSubview:buttonSendSum];
    [self.view addSubview:buttonSend];
    [self.view addSubview:buttonBack];
    [self.view addSubview:buttonImageCheckbox];
    [self.view addSubview:labelImageSize];
    [self.view addSubview:buttonImageCheckboxLarger];
    [self.view addSubview:labelTitle];

    if (self.displaySelectionButtons)
    {
        selectedButton = [UIButton buttonWithType:UIButtonTypeCustom];
        [selectedButton setImage:[UIImage imageNamed:@"choose-circle-o"]  forState:UIControlStateNormal];
        [selectedButton setImage:[UIImage imageNamed:@"choose_circle_n"]  forState:UIControlStateSelected];
        ...
        selectedButton.frame = CGRectMake([UIScreen mainScreen].bounds.size.width - 50, 22, 45, 45);
        [selectedButton setImageEdgeInsets:UIEdgeInsetsMake(12, 12, 12, 12)];
    ...
    // 如果之前进来选择过原图，那么再次进入就要判断之前的选择，如果有变化则刷新
    if (buttonImageCheckbox.selected != self.bOriginal)
    {
        [self buttonCheckboxPressed:nil];
    }
    
    labelImageSize.text = buttonImageCheckbox.selected ? [NSString stringWithFormat:ASLocalizedString(@"Check_for_original_size"), [self transformedValue:[self getTotalSize]]] : ASLocalizedString(@"Check_for_original");

   
    if (self.bSinlgePhotoMode)
    {
        selectedButton.alpha = 0;
        
        [self updateButtonSendSum];
//        [self setPhotoSelected:YES atIndex:0];
...
{
    __weak MWPhotoBrowser  *weakSelf = self;
    __block NSInteger totalSize = 0;
    [_photos enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if([weakSelf.selections[idx] boolValue])
        {
            MWPhoto *photo =  [weakSelf photoAtIndex:idx];
            NSData *imageData = UIImageJPEGRepresentation([weakSelf imageForPhoto:photo], 1.f);
...
- (void)performLayout
{
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        _doneButton = [[UIBarButtonItem alloc] initWithTitle:ASLocalizedString(@"KDCompanyChoseViewController_complete") style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)];
        // Set appearance
...
//            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:ASLocalizedString(@"KDApplicationQueryAppsHelper_tips")message:ASLocalizedString(@"照片流不支持预览")delegate:self cancelButtonTitle:ASLocalizedString(@"Global_Sure")otherButtonTitles:nil, nil]; [alert show];
        }
        // Update nav
        [self updateNavigation];
    }
...
//            [page.selectedButton removeFromSuperview];
    ...
    for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
        if (![self isDisplayingPageForIndex:index] || bShouldUpdatePageTiles) {
...
//                UIBarButtonItem *bbiSend = [[UIBarButtonItem alloc] initWithCustomView:selectedButton];
//                self.navigationItem.rightBarButtonItem = bbiSend;
...
- (CGRect)frameForSelectedButton:(UIButton *)_selectedButton atIndex:(NSUInteger)index {
    CGRect pageFrame = [self frameForPageAtIndex:index];
    CGFloat yOffset = 0;
    if (![self areControlsHidden]) {
        UINavigationBar *navBar = self.navigationController.navigationBar;
...
#endif
    CGRect captionFrame = CGRectMake(pageFrame.origin.x + pageFrame.size.width - 20 - _selectedButton.frame.size.width,
                                     statusBarOffset + yOffset,
                                     _selectedButton.frame.size.width,
                                     _selectedButton.frame.size.height);
    return CGRectIntegral(captionFrame);
}
...
////    NSData *imageData = UIImageJPEGRepresentation([self imageForPhoto:photo], all_chat_send_photo_compress_ratio);
////    NSData *imageData = UIImagePNGRepresentation([self imageForPhoto:photo]);
//     NSData *imageData = UIImageJPEGRepresentation([self imageForPhoto:photo], 1.f);
    labelImageSize.text = buttonImageCheckbox.selected ? [NSString stringWithFormat:ASLocalizedString(@"Check_for_original_size"), [self transformedValue:[self getTotalSize]]]:ASLocalizedString(@"Check_for_original");
}
...
- (void)updateNavigation {
    ...
    if (_gridController) {
        if (_gridController.selectionMode) {
            self.title = NSLocalizedString(ASLocalizedString(@"相机胶卷"), nil);
        } else {
            NSString *photosText;
            if (numberOfPhotos == 1) {
        ...
        if ([_delegate respondsToSelector:@selector(photoBrowser:titleForPhotoAtIndex:)]) {
            ...
            labelTitle.text = [NSString stringWithFormat:@"%lu %@ %lu", (unsigned long)(_currentPageIndex+1), NSLocalizedString(@"/", @"Used in the context: 'Showing 1 of 3 items'"), (unsigned long)numberOfPhotos];
...
- (void)selectedButtonTapped:(id)sender {
    ...
    if (!_selectedButton.selected && ([self calculateSelectedSum] >= 9))
    {
        [self showError];
        return;
    }
    ...
    for (MWZoomingScrollView *page in _visiblePages) {
        if (page.selectedButton == _selectedButton) {
            index = page.index;
            break;
        }
    }
    if (index != NSUIntegerMax) {
        BOOL btemp = [self.selections[index] boolValue];
        [self.selections replaceObjectAtIndex:index withObject:@(!btemp)];
         [self updateButtonSendSum];
        [self setPhotoSelected:_selectedButton.selected atIndex:index];
        [self updateImageSize];
    }
}
...
- (void)showError
{
    [KDErrorDisplayView showErrorMessage:ASLocalizedString(@"PostViewController_Alert")inView:self.view];
}
...
- (void)showGrid:(BOOL)animated {
    ...
    [UIView animateWithDuration:animated ? .25 : 0 animations:^(void) {
        _gridController.view.frame = self.view.bounds;
...
- (void)setCurrentPhotoIndex:(NSUInteger)index {
    ...
    if ([self isViewLoaded]) {
        ...
        if (!_viewIsActive || bShouldUpdatePageTiles)
...
// 单击页面，隐藏自定义按钮
- (void)setCustomViewHidden
{
    bHideCustomViews = !bHideCustomViews;
    
    viewNavigationBarBG.hidden = bHideCustomViews;
    viewToolbarBG.hidden = bHideCustomViews;
    buttonSend.hidden = bHideCustomViews;
    buttonBack.hidden = bHideCustomViews;
    buttonImageCheckbox.hidden = bHideCustomViews;
    labelImageSize.hidden = bHideCustomViews;
    buttonImageCheckboxLarger.hidden = bHideCustomViews;
    selectedButton.hidden = bHideCustomViews;
    buttonSendSum.hidden = bHideCustomViews;
    labelTitle.hidden = bHideCustomViews;
...
// 点击原图
- (void)buttonCheckboxPressed:(UIButton *)button
{
    
    // 20150313 新需求，如果原图按钮“未点击》点击”。图片没有勾选，勾选之
    if (!self.bSinlgePhotoMode && !buttonImageCheckbox.selected && !selectedButton.selected) {
        [self selectedButtonTapped:selectedButton];
    }
    
    [KDEventAnalysis event:event_image_original];
    buttonImageCheckbox.selected = !buttonImageCheckbox.selected;
    labelImageSize.alpha = buttonImageCheckbox.selected ? 1 : 1;
    buttonImageCheckbox.alpha = buttonImageCheckbox.selected ? 1 : 1;
    
    if([self.delegate respondsToSelector:@selector(photoBrowser:didChangeOriginalStates:)])
    {
        [self.delegate photoBrowser:self didChangeOriginalStates:buttonImageCheckbox.selected];
    }
    [self updateImageSize];
...
// 点击发送
- (void)buttonSendPresssed:(UIButton *)button
{
    if ([self calculateSelectedSum] == 0)
        return;
    
    if ([_delegate respondsToSelector:@selector(photoBrowserDidPressSendButton:)])
    {
        [self.delegate photoBrowserDidPressSendButton:self];
    }
...
{
    [buttonSendSum setTitle:@([self calculateSelectedSum]).description forState:UIControlStateNormal];
    if ([self calculateSelectedSum] == 0)
    {
        buttonSendSum.hidden = YES;
    }
    else
    {
        buttonSendSum.hidden = NO;
        
    }
}

// 计算发送“总数”
- (NSInteger)calculateSelectedSum
{
    int iSum = 0;
    for (NSNumber *number in self.selections) {
        if (number.boolValue) {
            iSum++;
        }
    }
