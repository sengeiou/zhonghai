...
+ (void) downloadsWithAttachemnts:(KDAttachment *)attachemnt finishBlock:(detectionFinishedBlock)finishBlock
{
    NSMutableArray *array = [NSMutableArray arrayWithCapacity:1];
    KDDownload *download = nil;
//    for (KDAttachment *attachment in attachemnts) {
        download = [[KDDownload alloc] init];
        download.downloadId = attachemnt.fileId;
        download.url = attachemnt.url;
        download.mimeType = attachemnt.contentType;
        download.name = attachemnt.filename;
        download.maxByte = attachemnt.fileSize;
        download.entityType = KDDownloadEntityTypeStatus;
        [download loadPath];
