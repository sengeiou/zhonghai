...
- (void) setPromptOn:(BOOL)on animated:(BOOL)animated;
- (CGRect) thumbFrameForState:(BOOL)isOn;
- (CGRect) promptFrameForState:(BOOL)isOn;
@end
...
- (void) configure {
    ...
    _promptLayer = [CATextLayer layer];// retain];
    _promptLayer.backgroundColor = [UIColor clearColor].CGColor;
    _promptLayer.font = (__bridge CFTypeRef _Nullable)(@"Helvetica");
    _promptLayer.fontSize = 16;
    _promptLayer.frame = [self promptFrameForState:_on];
    _promptLayer.string = ASLocalizedString(@"关");
    _promptLayer.alignmentMode = kCAAlignmentCenter;
    _promptLayer.foregroundColor = [UIColor whiteColor].CGColor;
    _promptLayer.contentsScale = [[UIScreen mainScreen] scale];
...
- (void)setOn:(BOOL)on animated:(BOOL)animated {
    ...
    [self setPromptOn:_on animated:animated];
}
...
- (void) setPromptOn:(BOOL)on animated:(BOOL)animated
{
    ...
    _promptLayer.frame = [self promptFrameForState:on];
    _promptLayer.string = on ? ASLocalizedString(@"开"): ASLocalizedString(@"关");
...
- (void) setPromptForegroundColor:(UIColor *)promptForegroundColor
{
    _promptLayer.foregroundColor = [promptForegroundColor CGColor];
}

- (UIColor *) promptForegroundColor
{
    return [UIColor colorWithCGColor:_promptLayer.foregroundColor];
}
...
- (CGRect) promptFrameForState:(BOOL)isOn {
    return CGRectMake(isOn ? 10.0 : self.bounds.size.width - 16.0 - 10.0,
                      (self.bounds.size.height - 16.0)/2 - 1,
                      16.0,
                      16.0);
}
...
//    [_promptLayer release],_promptLayer = nil;
    //[super dealloc];
}
